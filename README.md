...
### 2. The domain — describing the theme and the questions that the platform must answer in two to three paragraphs.
...
Answering these three question clusters gives exactly what's needed to move from requirements to an ER diagram, and then mechanically into DDL.
...
People and their matching data are kept separate. RIDERS holds stable identity info; RIDER_PREFERENCES is a 1:1 side table holding the volatile matching attributes (music, conversation, temperature, pooled tolerance). Drivers carry their matching attributes (languages, conversation_pref, home_area) directly on DRIVERS. Matching works by comparing the two sides — such as a rider's conversation_pref against driver's.

Too many-to-many relationships resolve through junctions. DRIVER_VEHICLES lets a driver operate multiple cars and a car be shared between drivers, with effective_from dating each pairing. DRIVER_BADGE_AWARDS connects drivers to the DRIVER_BADGES catalog — the catalog defines what badges exist, the junction records who earned which one and when.
